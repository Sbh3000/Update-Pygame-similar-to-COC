...
class Healer:
    ...
    def attack_target(self, target, isKing):
        ...
        troops = barbarians+ archers + dragons + balloons + stealth_archers
        for row in range(i-self.attack_radius, i + self.attack_radius):
            for col in range(j-self.attack_radius, j + self.attack_radius):
                if(row < 0 or col < 0):
                    continue
                for troop in troops:
                    if(troop.position[0] == row and troop.position[1] == col):
                        troop.deal_damage(-1 * self.attack)
...
def move_Healer(V,King):
    for hl in healers:
        if(hl.alive == False):
            continue
        troops = barbarians + dragons + archers + stealth_archers + balloons 
        closest_troop = search_for_closest_troop(hl.position, troops , King)
        if(closest_troop == None):
            continue
        hl.move(closest_troop, V , King)
...
def search_for_closest_troop(pos, troops , King):
    closest_troop = None
    min_distance = float('inf')  

    for troop in troops:
        if (troop.health < troop.max_health):
            distance = abs(troop.position[0] - pos[0]) + abs(troop.position[1] - pos[1])
            if distance < min_distance:
                closest_troop = troop
                min_distance = distance
    if King.alive == False:
        return
    if (abs(King.position[0]-pos[0])+abs(King.position[1]-pos[1]) < min_distance and King.health < King.max_health):
        return King.position
    return closest_troop.position if closest_troop else None
